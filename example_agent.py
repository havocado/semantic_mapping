...
topdown_position = sim.last_state().position[1:3]
location = np.zeros(3)
location[0] = topdown_position[1] * (-1)
...
def _action(sim):
  ...
  for act_no in range(num_acts):
    action_code = random.randint(0,2)
    if action_code == 0:
    ...
    location[0] = topdown_position[1] * (-1)
    location[1] = topdown_position[0] * (-1)
    print("retrieved position: ", sim.last_state().position)
